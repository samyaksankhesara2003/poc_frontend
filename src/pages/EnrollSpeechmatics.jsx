...
const ENROLL_URL = `${API_BASE.replace(/\/$/, "")}/speechmatics-enroll`;
const WS_SPEECHMATICS_URL =
  import.meta.env.VITE_WS_SPEECHMATICS_URL ||
  "ws://localhost:3000/speechmatics-conversation";
...
export default function EnrollSpeechmatics() {
  ...
  const [enrollmentResult, setEnrollmentResult] = useState(null); // { speaker_identifiers: string[], label: string }
  ...
  const canCloseSession =
    sessionStatus === SESSION_STATUS.ACTIVE || sessionStatus === SESSION_STATUS.PAUSED;
  const canStartRecording =
    sessionStatus === SESSION_STATUS.ACTIVE && !isRecording && enrollmentResult;
  ...
  const sendEnrollmentAudio = useCallback(async (blob) => {
    ...
    try {
      ...
      const res = await fetch(ENROLL_URL, { method: "POST", body: formData });
        ...
        if (last && last.speaker === seg.speaker) {
          ...
          if (cleanPart) {
            transcript[transcript.length - 1] = {
              ...last,
              text:
                last.text + (isPunctuationOnly(cleanPart) ? "" : " ") + cleanPart,
            };
          }
        } else {
  ...
  const startConversationRecording = useCallback(async () => {
    if (
      !enrollmentResult?.speaker_identifiers ||
    ...
    const ws = new WebSocket(WS_SPEECHMATICS_URL);
    wsRef.current = ws;
    ...
    ws.onopen = async () => {
      try {
        ws.send(
          JSON.stringify({
            type: "config",
            speaker_identifiers: enrollmentResult.speaker_identifiers,
          })
        );
    ...
    ws.onerror = () =>
      setConversationError("WebSocket error. Is the backend running?");
    ws.onclose = () => {
      setConnectingAudio(false);
      wsRef.current = null;
    };
  ...
  return (
    <div style={containerStyle}>
      <h2>Speechmatics speaker identification (enrollment only)</h2>
      <p style={{ fontSize: 14, color: "#666", marginBottom: 20 }}>
        Uses <strong>Speechmatics only</strong>: enroll the waiter with get_speakers, then start
        the session and conversation. Speechmatics returns &quot;Waiter&quot; for the enrolled
        speaker and S1/S2 for others — no Pinecone.
      </p>

      <section style={sectionStyle}>
        <div style={sectionTitle}>1. Enroll waiter voice (Speechmatics get_speakers)</div>
        <div style={buttonRow}>
          <button
            type="button"
            ...
            style={{
              ...primaryButton,
              ...(sessionStatus === SESSION_STATUS.ACTIVE ? buttonDisabled : {}),
            }}
          >
            + Create session
          </button>
          <button
            ...
            style={{
              ...secondaryButton,
              ...(enrollmentStatus === "recording" || enrollmentStatus === "sending"
                ? buttonDisabled
                : {}),
            }}
          >
          ...
          <button
            ...
            style={{
              ...secondaryButton,
              ...(enrollmentStatus !== "recording" ? buttonDisabled : {}),
            }}
          >
            Stop
          </button>
        ...
        {enrollmentStatus === "recording" && (
          <div style={timerStyle}>
            Recording: {formatDuration(recordDuration)} — aim for 5–30 s
          </div>
        )}
        {enrollmentStatus === "sending" && (
          <div style={statusStyle}>Enrolling with Speechmatics…</div>
        )}
        {enrollmentError && <div style={errorStyle}>{enrollmentError}</div>}
        {enrollmentStatus === "success" && enrollmentResult && (
          <div style={successBox}>
            Enrolled. <strong>Speaker identifiers</strong> received ({enrollmentResult.speaker_identifiers?.length || 0}). Start session and record conversation.
          </div>
        )}
      </section>
      ...
      {enrollmentResult && (
        <section style={sectionStyle}>
          <div style={sectionTitle}>2. Start session & conversation</div>
          <div style={buttonRow}>
            <button
              type="button"
          ...
          <div style={transcriptHint}>
            {sessionStatus === SESSION_STATUS.ACTIVE &&
              !isRecording &&
              "Click Start record to capture conversation."}
            {sessionStatus === SESSION_STATUS.ACTIVE &&
              isRecording &&
              (connectingAudio ? "Connecting…" : "Recording…")}
            {sessionStatus === SESSION_STATUS.PAUSED &&
              "Session paused. Start session again to continue."}
          ...
          {conversationTranscript.length === 0 ? (
            <div style={emptyTranscript}>
              {enrollmentResult
                ...
                : "Enroll waiter voice first (step 1)."}
            </div>
          ) : (
